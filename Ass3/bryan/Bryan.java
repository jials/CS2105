...
// Bryan receives AES session key from Amy
// Bryan reads messages line by line from file
// Bryan sends encrypted messages to Amy

class Bryan {  // Bryan is a TCP server
    
    int port;  // port Bryan listens to
    ServerSocket welcomeSkt;  // wait for Amy to connect
    Socket connectionSkt;     // socket used to talk to Amy
    private ObjectInputStream fromAmy;  // to read session key from Amy
    private ObjectOutputStream toAmy;   // to send encrypted messages to Amy
    ...
    public static void main(String[] args)  {
        ...
        if (args.length != 1) {
            System.err.println("Usage: java Bryan BryanPort");
            System.exit(1);
        }
    ...
    public Bryan(String portStr) {
        ...
        System.out.println("Bryan listens at port " + this.port);
        ...
        try {
            this.toAmy = new ObjectOutputStream(this.connectionSkt.getOutputStream());
            this.fromAmy = new ObjectInputStream(this.connectionSkt.getInputStream());
        } catch (IOException ioe) {
    ...
    sendPublicKey();
        ...
        // Send encrypted messages to Amy
        sendMessages();
    ...
    // Send public key to Amy
    public void sendPublicKey() {
        try {
            ObjectInputStream ois = 
                    new ObjectInputStream(new FileInputStream("bryan.pub"));
            PublicKey pubKey = (PublicKey)ois.readObject();
            ois.close();

            byte[] digest = Files.readAllBytes(Paths.get("bryan.sig"));

            this.toAmy.writeObject(pubKey);
            this.toAmy.writeObject(digest);
        } catch (IOException ioe) {
            System.out.println("Error sending public key to Amy");
            System.exit(1);
    ...
    public void getSessionKey() {
        ...
        try {
            SealedObject sessionKeyObj = (SealedObject)this.fromAmy.readObject();
            this.crypto.setSessionKey(sessionKeyObj);
        } catch (IOException ioe) {
            System.out.println("Error receiving session key from Amy");
            System.exit(1);
        } catch (ClassNotFoundException ioe) {
    ...
    // Read messages one by one from file, encrypt and send them to Amy
    public void sendMessages() {
        ...
        try {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                this.toAmy.writeObject(encryptedMsg);
            }
            
            fromFile.close();  // close input file stream
            System.out.println("All 10 messages are sent to Amy");
            
        } catch (FileNotFoundException fnfe) {
            System.out.println("Error: " + MESSAGE_FILE + " doesn't exist");
            System.exit(1);
        } catch (IOException ioe) {
            System.out.println("Error sending messages to Amy");
            System.exit(1);
        }
    }
    ...
    class Crypto {
        ...
        // Session key is received from Amy
        private SecretKey sessionKey;
        // File that contains private key
        public static final String PRIVATE_KEY_FILE = "bryan.pri";
        ...
        public Crypto() {
            ...
            if ( privKeyFile.exists() && !privKeyFile.isDirectory() ) {
                ...
                System.out.println("Bryan cannot find RSA private key.");
                System.exit(1);
            }
        }
        
        // Receive a session key from Amy
        public void setSessionKey(SealedObject sessionKeyObj) {
            
            try {
                // getInstance(crypto algorithm/feedback mode/padding scheme)
                // Amy will use the same key/transformation
                Cipher cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding");
                cipher.init(Cipher.DECRYPT_MODE, this.privKey);
                
                // receive an AES key in "encoded form" from Amy
